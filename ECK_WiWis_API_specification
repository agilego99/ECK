...
- [APIs](#apis)
    ...
    - [5. wounds](#5-wounds)
        ...
        - [5.13. GET /organizations/{organization-id}/daypass-patients/charts/{chart-id}/wounds](#513-get-organizationsorganization-iddaypass-patientschartschart-idwounds)
        - [5.14. GET /organizations/{organization-id}/daypass-patients/charts/{chart-id}/wounds/{chart-wounds-id}](#514-get-organizationsorganization-iddaypass-patientschartschart-idwoundschart-wounds-id)
        - [5.15. GET /organizations/{organization-id}/daypass-patients/charts/{chart-id}/wounds/chart-wound-sub-part-combination/{chart-wound-sub-part-combination}](#515-get-organizationsorganization-iddaypass-patientschartschart-idwoundschart-wound-sub-part-combinationchart-wound-sub-part-combination)
    ...
    - [13. consultations](#13-consultations)
        ...
        - [13.10. POST /wounds/{chart-wound-id}/consultations/consultation-window](#1310-post-woundschart-wound-idconsultationsconsultation-window)
        - [13.11. GET /users/{user-id}/daypass-patients/consult-count](#1311-get-usersuser-iddaypass-patientsconsult-count)
    ...
    - [18. daypass-patients](#18-daypass-patients)
        ...
        - [18.3. PUT /users/{patient-id-no}/daypass-patients-hospital/effective-period](#183-put-userspatient-id-nodaypass-patients-hospitaleffective-period)
        ...
        - [18.6. POST /daypass-patients-hospital](#186-post-daypass-patients-hospital)
        - [18.7. PUT /users/{patient-id-no}/daypass-patients-hospital/is-pause](#187-put-userspatient-id-nodaypass-patients-hospitalis-pause)
...
[
  ...
  {
    "patient_status_value": "emergency",
    "patient_status_description": "急診"
  },  
  {
...
|Name|Data Type|Description|Example|
|:---|:---|:---|:---|
|patient_status_value|string|病患狀態下拉選單代碼|admission<br>day_pass<br>home_care<br>discharge<br>emergency|
|patient_status_description|string|病患狀態下拉選單說明|住院中<br>DayPass<br>居家照護<br>急診<br>出院|
...
|option|emergency|bool|查詢急診的病患資料|true<br>false|
...
|patient_status_description|string|病患狀態|住院<br>DayPass<br>居家照護<br>急診<br>出院<br>空白(未知)(畫面顯示--)|
...
|hospital_bed_no|string|病床號碼|1003-3|
|patient_status|string|病患狀態|admission<br>day_pass<br>home_care<br>discharge<br>emergency|
|patient_status_description|string|病患狀態說明|住院<br>DayPass<br>居家照護<br>急診<br>出院|
...
|option|query|string|模糊查詢病患資料(床號，身份證字號，姓名，住院、居家、DayPass、急診、出院，男、女)|王O明<br>A123456789|
|option|first|int|回傳開頭的前N筆資料|10|
|option|after|string|回傳該curosr後面的資料。一定要搭配first|98a4f867-8dcd-4982-aa3a-14e1030bcd88|
|option|last|int|回傳倒數的N筆資料|10|
|option|before|string|回傳該curosr前面的資料。一定要搭配last|98a4f867-8dcd-4982-aa3a-14e1030bcd88|
...
|Pagination|N/A|
|Access Control|恩主公醫師、恩主公護理師、恩主公高級管理者、恩主公一般管理者、長照機構護理師、長照機構高級管理者、長照機構一般管理者、DayPass 病患|
...
|visiting_staff_name|string|主治醫師姓名|陳Ｏ誠|
|note|string|備註||
|patient_status|string|病患狀態|admission<br>day_pass<br>home_care<br>emergency<br>discharge|
|patient_status_description|string|病患狀態說明|住院<br>DayPass<br>居家照護<br>急診<br>出院|
|health_behavior_histories|array of health_behavior_history|個人史|無資料時回傳空array|
|medical_histories|array of medical_history|疾病史|無資料時回傳空array|
|operation_history|string|手術史||
...
待確認：
	- patient_status(病患狀態)增加emergency(急診)選項，判斷方式還需詢問資訊室治銘。
  
2.10 與 2.14合併，之後"掃碼新增"和"掃描病患"都用2.10。
- 不論"掃碼新增"和"掃描病患"，都需要跟HIS要資料，所以data_source(資料來源)都為"his"，若HIS無此資料則回傳錯誤。
- 若為"無名氏"，病患的身分證字號、性別、年齡、生日...來自HIS的資料可能為空，這是正常的。
...
- 因應流程修改：
	...
	- 存在 HIS，但病患清單及工作清單無此病患
		- 直接回傳HIS資料，is_chart和is_work_item欄位回傳false，然後前端編輯完基本資料後，再分別呼叫新增病患和新增工作清單的API。

	- 存在 HIS，且有病患清單，但沒有在工作清單
		- 此時須將HIS的資料更新回病患清單，但是新增工作清單的動作交給前端呼叫另一支API。
...
|require|patient_status_description|string|病患狀態|住院<br>急診<br>出院<br>空白(未知)|
...
|patient_telephone|string|電話|0987654321|
|hospital_bed_no|string|病床號碼|1003-3|
|patient_status_description|string|病患狀態|住院<br>急診<br>出院<br>空白(未知)|
...
- 未完成的計算規則為登入的使用者自己所建立的傷口記錄，非自己所建立的傷口記錄，is_finished會一律設為true
...
|option|query|string|模糊查詢病患資料|王O明<br>A123456789|
|option|sortby|string|按照哪個屬性排序|hospital_bed_no<br>unfinished<br>last_update_time<br>care_items<br>admission<br>day_pass<br>home_care<br>discharge<br>emergency|
|option|first|int|回傳開頭的前N筆資料|10|
|option|after|string|回傳該curosr後面的資料。一定要搭配first|98a4f867-8dcd-4982-aa3a-14e1030bcd88|
|option|last|int|回傳倒數的N筆資料|10|
|option|before|string|回傳該curosr前面的資料。一定要搭配last|98a4f867-8dcd-4982-aa3a-14e1030bcd88|
...
|patient_birthday|date|生日|1980-01-01|
|hospital_bed_no|string|病床號碼|1003-3|
|patient_status|string|病患狀態|admission<br>day_pass<br>home_care<br>emegency<br>discharge|
|patient_status_description|string|病患狀態說明|住院<br>DayPass<br>居家照護<br>急診<br>出院|
|last_update_time|datetime|更新日期|2019-07-18T13:54:37.917+0000|
|is_care_item|bool|是否在關懷清單|true<br>false|
|care_item_id|string|關懷清單ID|2a41bf3a-afde-1297-2e13-b1e1b8f4a3e5|
...
|require|user-id|string|使用者ID|98a4f867-8dcd-4982-aa3a-14e1030bcd88|
|option|query|string|模糊查詢病患資料|王O明<br>A123456789|
|option|sortby|string|按照哪個屬性排序|hospital_bed_no<br>unfinished<br>last_update_time<br>work_items<br>admission<br>day_pass<br>home_care<br>discharge<br>emegency|
|option|first|int|回傳開頭的前N筆資料|10|
|option|after|string|回傳該curosr後面的資料。一定要搭配first|98a4f867-8dcd-4982-aa3a-14e1030bcd88|
|option|last|int|回傳倒數的N筆資料|10|
|option|before|string|回傳該curosr前面的資料。一定要搭配last|98a4f867-8dcd-4982-aa3a-14e1030bcd88|
...
|hospital_bed_no|string|病床號碼|1003-3|
|patient_status|string|病患狀態|admission<br>day_pass<br>home_care<br>discharge<br>emergency|
|patient_status_description|string|病患狀態說明|住院<br>DayPass<br>居家照護<br>出院<br>急診|
|last_update_time|datetime|更新日期|2019-07-18T13:54:37.917+0000|
|is_work_item|bool|是否在工作清單|true<br>false|
...
|Pagination|N/A|
|Access Control|恩主公醫師、恩主公護理師、恩主公高級管理者、恩主公一般管理者、長照機構護理師、長照機構高級管理者、長照機構一般管理者。<br>額外驗證：{user-id}只能為自己(用token驗證)、DayPass 病患|
...
|wound_hypotenuse|numeric|傷口斜邊長(mm)|120|
...
|require|wound_hypotenuse|numeric|傷口斜邊長(mm)|120|
...
### 5.13. GET /organizations/{organization-id}/daypass-patients/charts/{chart-id}/wounds

|欄位|描述|
|:---|:---|
|Description|查詢傷口紀錄列表(DayPass病患)|
|Pagination|N/A|
|Access Control|DayPass病患|
|Using|App->傷口紀錄|

**Note**
- 取得 DayPass 傷口部位時，需取得醫護人員建立的所有部位，排除未分類部位
- 顯示 DayPass 照片時，僅取得該病患自己建議之照片
- 未分類的資料chart_wound_sub_part_combination=0，chart_wound_sub_part_combination_description=""
- 組合資料時，chart_wound_sub_part_combination將所有子部位組合起來，但是chart_wound_sub_part_combination_description會將中文名稱重複的過濾掉，例如"右大拇指"有分前後的ID，但是description都是"右大拇指"，這時候只要顯示一次，不要顯示成"右大拇指+右大拇指"
- 組合資料時，依據wound_sub_parts的index欄位，決定chart_wound_sub_part_combination和chart_wound_sub_part_combination_description的組合順序
- DayPass病患查詢時，chart_wonds內creator不是DayPass病患本人的資料不顯示，外層分類資料夾不受此影響，只要該資料夾有資料，就會有資料夾的結構。
- 查詢有包含這個部位的資料夾，非完全等於此部位的傷口資料夾
- DayPass
    - 是否已提出諮詢
...
### 5.14. GET /organizations/{organization-id}/daypass-patients/charts/{chart-id}/wounds/{chart-wounds-id}

|欄位|描述|
|:---|:---|
|Description|查詢傷口詳細資料(DayPass病患)|
|Pagination|N/A|
|Access Control|DayPass病患|
|Using|App->傷口列表->傷口紀錄|
...
### 5.15. GET /organizations/{organization-id}/daypass-patients/charts/{chart-id}/wounds/chart-wound-sub-part-combination/{chart-wound-sub-part-combination}

|欄位|描述|
|:---|:---|
|Description|查詢傷口紀錄之該部位傷口列表(DayPass病患)|
|Pagination|N/A|
|Access Control|DayPass病患|
|Using|App->傷口列表->傷口紀錄|
...
**Note**
* 當刪除諮詢對象已被設定為長照機構的諮詢窗口人員時，則無法對該對象進行刪除。
    * 上述無法刪除，需有對應畫面顯示無法刪除原因。
***
**Request Header**
...
* asker_description
* 當提出者為恩主公員工時，顯示職稱
* 當提出者為DayPass ，顯示 DayPass
* 當提出者為長照機構護理師時，顯示長照機構名稱
* DayPass 病患禁止向上諮詢
    * 使用 user_id 查詢 daypass_patients.user_id
        * is_daypass_asker = true 表示 DayPass 病患
* 當為 DayPass 病患，則判斷過期狀態，在 is_daypass_expired 輸入相段應 boolean 值。
...
```json
{
  "edges": [
    ...
    },
    {
      ...
      "is_ask_higher_consultant":false, 
      "is_new": true,
      "is_daypass_asker": true,
      "is_daypass_expired": true,
      "last_update_time": "2019-07-18T13:54:37.917+0000",
      "status": "replied"
    }
  ],
...
|asker_employee_name|string|提出諮詢者員工姓名<br>(長照機構/DayPass 撈 user_name欄位的資料)|林美玲|
...
|is_daypass_asker|bool|是否為DayPass 病患|true 為 DayPass 病患 <br> false 或空白表示非 DayPass 病患|
|is_daypass_expired|bool|DayPass 病患使用狀態已過期|true 表已過期 <br> false 表未過期 <br> 空 表非 DayPass 病患，因此無使用狀態 |
|last_update_time|datetime|更新日期|2019-07-18T13:54:37.917+0000|
...
* DayPass 病患禁止向上諮詢
    * 使用 user_id 查詢 daypass_patients.user_id
        * is_daypass_asker = true 表示 DayPass 病患 提出諮詢
    * 如為 DayPass 病患，則判斷是否已過期
        * is_daypass_expired = true 表示 DayPass 病患 已過期
...
|patient_age|int|年齡|30|
|patient_birthday|date|生日|1980-01-01|
|is_daypass_asker|boolean|是否為 DayPass 病患|true 表 DayPass 病患<br>false 表非 DayPass 病患|
|is_daypass_expired|bool|DayPass 病患使用狀態已過期|true 表已過期 <br> false 表未過期 <br> 空 表非 DayPass 病患，因此無使用狀態 |
|is_work_item|bool|是否在工作清單|true<br>false|
|work_item_id|string|工作清單ID|98a4f867-8dcd-4982-aa3a-14e1030bcd88|
|is_care_item|bool|是否在關懷清單|true<br>false|
...
|employee_name|string|員工姓名<br>(長照機構/DayPass 撈 user_name欄位的資料)|王小明|
|position_description|string|職稱說明|醫師|
|message|string|訊息內容|應為XXX狀況，建議改用以下敷料|
...
* 如果是 DayPass 病患，以下兩種狀態，可對主治醫師提出諮詢，但每日諮詢次數以3次為限
    * 使用中
    * 重啟服務
    * 上述兩種狀態，須檢查 DayPass 病患當日諮詢次數。每日諮詢次數以3次為限，若超過3次（含），則不允許 DayPass 病患提出諮詢
        * 查詢 consultations
        * 比對提出諮詢者的使用者ID 及提出資詢日期，運算當日已諮詢次數
    * 當提出者為 DayPass 病患，須填入 "已向主治醫師提出諮詢" 固定諮詢語句
...
|Description|向上諮詢|
|Pagination|N/A|
|Access Control|恩主公醫師(不包含傷口專家)、傷口個管師、諮詢窗口|
...
### 13.11. GET /users/{user-id}/daypass-patients/consult-count

|欄位|描述|
|:---|:---|
|Description|DayPass 病患當日已諮詢次數|
|Pagination|N/A|
|Access Control|DayPass病患|
|Using|App->傷口列表->傷口紀錄->提出諮詢|

**Note**
* DayPass 病患當日已諮詢次數查詢
   * 比對提出諮詢者的使用者ID 及 提出諮詢日期
      * daypass_consltations_count
      * 查詢 consultations
      * 比對提出諮詢者的使用者ID 及 提出諮詢日期，運算當日已諮詢次數
...
```json
{
 "daypass_consultations_count": 2,
 "daypass_consultations_count_max_limit": 3
}
...
|Name|Data Type|Description|Example|
|:---|:---|:---|:---|
|daypass_consultations_count|int|根據提出諮詢日已提出次數|2|
|daypass_consultations_count_max_limit|int|允許最大諮詢次數（包含）|3|
...
* 登入流程
    * DayPass病患
        ...
        * 產生傷口案用的Token
            * 有別於其它 Token ，DayPass 病患的 Token 效期為無期限
        * DayPass 使用狀態
            * HTTP 200
                * DayPass 可登入
                    * daypass-status-key = 0
                    * daypass-status-info 為空值
                * DayPass 已過期
                    * daypass-status-key =1
                    *  daypass-status-info 為空值
                * 帳號暫停使用
                    * daypass-status-key =2
                    * daypass_reserve_period = 暫停剩餘保留天數
                        * 取得 暫停剩餘保留天數
                * 傷口資料夾尚未開啟，以致 DayPass 無法登入
                    * 已建立資料夾
                        * 使用 chart_id 已存在 chart_wounds 且 chart_wounds. chart_wound_sub_part_combination 內容非 0
                    * 尚未建立資料夾
                        * daypass-status-key =3
                        * daypass-status-info 為空值
            * PAYLOAD
                * DayPass 使用狀態
                    * 查詢 daypass_patients
                        * 判斷已過期
                            * 判斷 daypass_status = 4 為已過期
                        * 當未過期
                            * 判斷 daypass_status
                                * 3 = 暫停使用
                                * 4 = 重啟服務
                ...
                * 主治醫師員編改由 daypass_patients 表 visiting_staff_no 取得欄位內容
                    * 主治醫師姓名
                        * 關聯employees.employee_no，取得主治醫師姓名
                    * 主治醫師科名
                        * 用employees.division_id關聯divisions. division_name ，查詢主治醫師科別名稱
    * 長照機構人員
        * 檢查依據資料庫使用者帳號密碼做Login
        * 產生傷口案用的Token
    * 院內醫護人員
...
```json
{
  ...
  "daypass_status_key": 1,
  "daypass_reserve_period": 4,
  "error_message": ""
}
```
...
|result|bool|登入結果|true=成功<br>false=失敗|
|token|string|token||
|daypass_status_key|int|DayPass 使用狀態碼|0 表可登入<br>1 表已過期<br>2 表暫停使用<br>3 表傷口資料夾尚未開啟，以致 DayPass 無法登入|
|daypass_reserve_period|int|DayPass 剩餘天數||
|error_message|string|錯誤訊息|登入失敗|
...
```json
{
  ...
  "daypass_status_description": "使用中",
...
|daypass_status_description|string|DayPass 使用狀態|ex.<br>使用中<br>暫停使用<br>重啟服務<br>已過期<br>|
...
* 使用天數
    * 取得 使用天數
* 第二層：使用歷程
    * 根據 daypass_patient_id 查詢 daypass_patients_histories
        * 以根據觸發事件決定
            * 開始日期
                * 起：事件觸發日 ＋ 1
            * 續購日期
...
|require|is-paused|bool|暫停使用|true<br>false|
|require|is-resume|bool|重啟服務|true<br>false|
|option|query|string|模糊查詢(病患姓名、身分證字號、性別、生日、電話、主治醫師、帳號)||
|option|sortby|string|按照哪個屬性排序|patient_name<br>patient_id_no<br>patient_sex<br>patient_birthday<br>patient_telephone<br>visiting_staff_name<br>user_account<br>effective_period<br>expiration_date<br>last_update_time<br>daypass_status_discription<br>remain_days|
|option|order|string|排序順序 (需搭配sortby)|asc<br>desc|
|option|limit|int|一頁多少筆 (需搭配offset)|10|
|option|offset|int|從第幾筆開始 (需搭配limit)|0|
  ...
  "edges": [
    {
      ...
      "password": "a93e8c",
      "effective_period": 14,
      "daypass_status_discription": "使用中", 
      "remain_days": 10,
      "expire": {
        "effective_date": "2019-09-20",
        "expiration_date": "2019-09-30"
      },
        "daypass_behavior_histories":[
            {
                "daypass_history_status": "開始日期", 
                "date": "2010-01-12"

            },
            {
                "daypass_history_status": "暫停日期", 
                "date": "2010-11-21"
            },
            {
                "daypass_history_status": "重啟日期", 
                "date": "2010-11-24"
            },
            {
                "daypass_history_status": "結束日期", 
                "date": "2010-09-20"
            }
        ],
      "last_update_time": "2019-07-18T13:54:37.917+0000"
...
|daypass_status_discription|string|DayPass 使用狀態描述|使用中<br>重啟服務<br>暫停使用<br>重啟服務<br>|
|remain_days|int|剩餘天數<br>|
|daypass_behavior_histories|object of daypass_behavior_histories|DayPass 歷史使用狀態|ex.<br>開始日期<br>續購日期<br>暫停日期<br>重啟日期<br>結束日期<br>|
...
|Name|Data Type|Description|Example|
|:---|:---|:---|:---|
|daypass_history_status|date|DayPass 歷史使用狀態|ex.<br>開始日期<br>續購日期<br>暫停日期<br>重啟日期<br>結束日期<br>|
|date|date|日期|2019-10-03|
...
### 18.3. PUT /users/{patient-id-no}/daypass-patients-hospital/effective-period

|欄位|描述|
|:---|:---|
|Description|DayPass 病患續付及展延使用天數|
|Pagination|N/A|
|Access Control||
|Using||
...
**Note**
* 此 API 為院方呼叫
    * 至 eck_sso 驗證院方的 Token 是否有效
* 續付及展延
    * expiration_date 未過期且 daypass_status 為 新購或重啟服務
        * 新增 daypass_patients_histories
        * 更新 daypass_patients
...
|Require|Name|Data Type|Description|Example|
|:---|:---|:---|:---|:---|
|require|patient-id-no|String|身分證號|A123456789|
...
```json
{
  "authorization":"eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.JmX8A…",
  "effective_period": 5,
  "visiting_staff_no": "023456789"
}
```

|Require|Name|Data Type|Description|Example|
|:---|:---|:---|:---|:---|
|require|authorization|strinng|token||
|require|effective_period|int|使用天數|3,5,7,10,14(天)|
|require|visiting_staff_no|string|主治醫師員工編號|023456789|
...
```json
{
  "result": false,
  "error_code": 1,
  "error_message": "查無此 DayPass 病患，請改用新購 DayPass API"
}
```

|Name|Data Type|Description|Example|
|:---|:---|:---|:---|
|result|bool|處理結果|true 表成功 <br> false 表失敗|
|error_code|int|錯誤代碼|0 表 成功<br>1 表 此 DayPass 病患暫停使用中，請先執行重啟 DayPayPass API<br>2 表 此 DayPass 病患，已過期，請改用新購 DayPass API<br>3 表 無此 DayPass 病患，請改用新購 DayPass API|
...
|Pagination|N/A|
|Access Control|恩主公高級管理者、恩主公一般管理者|
|Using|Web->DayPass 管理|

**Note**
* password隨機產生一組6位"小寫英文＆數字"組合的值
* 至 eck_commone 調用院方簡訊系統 API
...
### 18.6. POST /daypass-patients-hospital

|欄位|描述|
|:---|:---|
|Description|由院方發動新增DayPass病患|
|Pagination|N/A|
|Access Control||
|Using||
...
* 此 API 為院方呼叫
    * 至 eck_sso 驗證院方的 Token 是否有效
* 驗證欲新增之 DayPass 病患身分證字號是否存在
* 有存在病患清單，但不存在HIS
    * 不可新增，回傳查無病患資料。(DayPass病患資料必須存在HIS)
* 有存在HIS，但不存在病患清單
    * 新增一筆病患資料(charts)
        * 備註、個人史、疾病史、手術史留空，其餘欄位使用HIS資料
* 同時存在病患清單及HIS
    * 備註、個人史、疾病史、手術史使用病患清單資料，其餘欄位使用HIS資料，此時資料來源應為his(從HIS新增)
* 在病患清單，將該筆資料的欄位 patient_status = day_pass
* 新增使用者資料(users)
    * employee_id, organization_id留空
    * user_account填入病患身分證字號
    * user_name 填入使用者姓名
...
* 更新使用者資料(users)
    * telephone 填入病患電話號碼
* 使用角色對應（users_roles_mapping）
    * user_id
    * role_id  根據 daypass_user 取得對應編號
* 新增病患資料 daypass_patients
    * 沒有該病患 user_id
        * 新增 daypass_patients
    * 已有該病患 user_id
        * 使用期限未過期
        ...
        * 使用期限已過期
            * 新增 daypass_patients
            * 新增 daypass_patients__histories
    * 生效日為隔天
    * 使用期限
        * 生效日加上使用天數計算出來
            * ex:生效日為2019-09-01，使用天數為10天，那使用期限經計算會是2019-09-10
* 透過18.7 API，調用院方簡訊系統；將 DayPass 相關資訊，透過院方簡訊系統發送給 DayPass 病患
...
```json
{
 ...
 "patient_id_no": "A123456789",
 "effective_period": 10,
 "visiting_staff_no": "123456789"
}
```

|Require|Name|Data Type|Description|Example|
|:---|:---|:---|:---|:---|
|require|authorization|String|token||
|require|patient_id_no|string|身分證字號|A123456789|
|require|effective_period|int|使用天數|3,5,7,10,14(天)|
|require|visiting_staff_no|String|主治醫師員工編號|123456789|
...
```json
{
  "result": false,
  "error_code": 1,
  "error_message": "此 DayPass 病患目前暫停使用"
}
```
...
|:---|:---|:---|:---|
|result|bool|處理結果|true 表成功 <br> false 表失敗|
|error_code|int|錯誤代碼|0 表 成功<br>1 表 此 DayPass 病患目前狀態為暫停使用<br>2 表 此 DayPass 病患目前狀態為使用中 <br>3 表 查無此病患資料|
|error_message|string|錯誤訊息||
...
### 18.7. PUT /users/{patient-id-no}/daypass-patients-hospital/is-pause

|欄位|描述|
|:---|:---|
|Description|暫停/重啟 DayPass 服務功能|
|Pagination|N/A|
|Access Control||
|Using||
...
* 此 API 為院方呼叫
    * 至 eck_sso 驗證院方的 Token 是否有效
* 身分證字號正確性檢查
* 查詢資料表（daypass_patients）
    * 不予暫停及重啟邏輯，滿足以下任一條件，則不允許本次操作
        * 已過期
            * 判斷 daypass_status = 4
        * 已暫停
            * 若 daypass_status = 2
...
|Require|Name|Data Type|Description|Example|
|:---|:---|:---|:---|:---|
|require|authorization|String|是否暫停|true 暫停<br>false 重啟|
|require|is-pasue|bool|是否暫停|true 暫停<br>false 重啟|
...
```json
{
  "result": false,
  "error_code": 1,
  "error_message": "此 DayPass 病患暫停使用中"
}
```
...
|:---|:---|:---|:---|
|result|bool|處理結果|true 表成功 <br> false 表失敗|
|error_code|int|錯誤代碼|0 表 成功<br>1 表 此 DayPass 病患暫停使用中<br>2 表 此 DayPass 病患目前狀態為使用中<br>3 表 此 DayPass 病患已過期<br>4 表 無此 DayPass 病患|
|error_message|string|錯誤訊息||
